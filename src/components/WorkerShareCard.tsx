...
const WorkerShareCard = ({ uid, name }: Props) => {
  ...
  const url = useMemo(() => {
    if (typeof window === "undefined") return `/w/${uid}`;
    return `${window.location.origin}/w/${uid}`;
  }, [uid]);
    ...
    <div className="rounded-2xl border border-border/60 bg-card p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="space-y-0.5">
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Your share link</p>
          <p className="font-mono text-sm text-primary">{uid}</p>
        </div>
        <button
          onClick={() => setShowQr((v) => !v)}
          className="rounded-full border border-border bg-muted px-3 py-1.5 text-xs font-semibold flex items-center gap-1.5 hover:bg-muted/70 transition"
        >
      ...
      <div className="flex items-center gap-2 rounded-lg border border-border/60 bg-muted/40 px-3 py-2">
        ...
        <button onClick={copy} className="text-muted-foreground hover:text-primary transition" aria-label="Copy link">
