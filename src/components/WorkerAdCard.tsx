    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* ── STATS GRID ────────────────────────────────────────── */}
          <div className="relative mx-3 mt-2 grid grid-cols-2 gap-1.5">
            {/* distance — hero stat */}
            <div className={`relative col-span-2 overflow-hidden rounded-2xl border ${t.border} bg-gradient-to-br ${t.soft} p-2.5`}>
              {/* faux map dashed path */}
              <svg className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 opacity-60" width="80" height="36" viewBox="0 0 90 42" fill="none">
              ...
              <div className="relative flex items-center gap-2.5">
                <div className={`flex h-9 w-9 items-center justify-center rounded-full border ${t.border} bg-black/50`}>
                  <MapPin className={`h-4 w-4 ${t.textStrong}`} />
                </div>
                <div className="min-w-0">
                  <div className="flex items-baseline gap-1 leading-none">
                    ...
                    {hasDistance && <span className="text-[10px] font-bold text-white/70">km</span>}
                  </div>
                  <div className={`mt-0.5 text-[9px] font-black uppercase tracking-[0.18em] ${t.textStrong}`}>Distance</div>
                  <div className="text-[9px] text-white/55">From your location</div>
                </div>
              </div>
            </div>

            {/* experience */}
            <div className="relative flex items-center overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] p-2">
              <div className="flex items-center gap-2">
                <div className={`flex h-8 w-8 items-center justify-center rounded-full border ${t.border} bg-black/50`}>
                  <CalendarClock className={`h-3.5 w-3.5 ${t.textStrong}`} />
                </div>
                <div className="min-w-0">
                  <div className="text-[16px] font-black leading-none text-white tabular-nums">
                    {worker.experience > 0 ? `${worker.experience}+` : "—"}
                  </div>
                  <div className={`mt-0.5 text-[8px] font-black uppercase tracking-[0.16em] ${t.textStrong}`}>Years Exp</div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-2">
              <div className={`mb-1 flex items-center gap-1 text-[8px] font-black uppercase tracking-[0.16em] ${t.textStrong}`}>
          ...
          {/* ── CTAs ──────────────────────────────────────────────── */}
          <div className="relative mx-3 mt-2 grid grid-cols-[1fr_1.5fr] gap-1.5" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => { fireClick(); setPopupOpen(true); }}
              className="group/btn flex items-center justify-between rounded-2xl border border-white/12 bg-white/[0.05] px-2.5 py-2 text-left transition hover:bg-white/[0.09]"
            >
              <div className="flex min-w-0 items-center gap-1.5">
                <User className={`h-3.5 w-3.5 shrink-0 ${t.textStrong}`} />
                <div className="min-w-0">
                  <div className="text-[10.5px] font-black uppercase tracking-[0.12em] text-white">Profile</div>
                  <div className="truncate text-[8px] text-white/55">View details</div>
                </div>
              </div>
              <ChevronRight className="h-3 w-3 shrink-0 text-white/60 transition group-hover/btn:translate-x-0.5" />
            </button>
                ...
                <>
                  <div className="flex min-w-0 items-center gap-1.5">
                    ...
                    <div className="min-w-0 text-left">
                      <div className="text-[11px] font-black uppercase tracking-[0.14em]">Message</div>
                      <div className="truncate text-[8px] font-semibold opacity-80">Chat with {firstName}</div>
                    </div>
                  </div>
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-black/85 text-white">
                    <ChevronRight className="h-3 w-3" />
                  </span>
                </>
              );
              const ctaClass = `group/btn flex w-full items-center justify-between rounded-2xl bg-gradient-to-r ${t.grad} px-2.5 py-2 text-black shadow-lg shadow-lime-500/15`;
          ...
          {/* ── FOOTER TRUST BAR ──────────────────────────────────── */}
          <div className={`relative mt-2 grid grid-cols-3 gap-1 border-t ${t.border} bg-black/50 px-3 py-2 backdrop-blur`}>
            <div className="flex flex-col items-center text-center">
              <ShieldCheck className={`h-3.5 w-3.5 ${t.textStrong}`} />
              <div className="mt-0.5 text-[8px] font-black uppercase tracking-[0.1em] text-white">Verified</div>
              <div className="text-[7.5px] leading-tight text-white/50">ID Checked</div>
            </div>
            <div className="flex flex-col items-center text-center">
              <Star className="h-3.5 w-3.5 fill-amber-400 text-amber-400" />
              <div className="mt-0.5 text-[9px] font-black text-white tabular-nums">
                {(worker.rating || 4.9).toFixed(1)}<span className="text-white/45">/5</span>
              </div>
              <div className="text-[7.5px] leading-tight text-white/50">Rating</div>
            </div>
            <div className="flex flex-col items-center text-center">
              <Trophy className="h-3.5 w-3.5 text-amber-400" />
              <div className="mt-0.5 text-[8px] font-black uppercase tracking-[0.1em] text-amber-300">Top Pro</div>
              <div className="text-[7.5px] leading-tight text-white/50">Near Konnect</div>
            </div>
          </div>
        </article>
      </div>
