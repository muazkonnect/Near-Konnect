...
type DonorRow = {
  ...
  phone: string | null;
  profile_contact_methods: { methods: unknown } | null;
  ...
  const { data: donors = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("profiles")
        .select("user_id, full_name, avatar_url, city, blood_group, latitude, longitude, blood_show_contact, profile_phones(phone), profile_contact_methods(methods)" as any)
          ...
          <div className="flex h-9 items-center">
            <div className="flex gap-10 whitespace-nowrap px-4" style={{ animation: `ticker ${tickerSpeed}s linear infinite` }}>
              {[...tickerItems, ...tickerItems, ...tickerItems].map((t, i) => (
                <span
                  key={i}
                  className={`flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-slate-50 ${
