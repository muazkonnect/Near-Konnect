...
BEGIN
  ...
  IF v_role = 'worker' AND v_profession IS NOT NULL AND v_main_category IS NOT NULL AND v_sub_category IS NOT NULL THEN
    INSERT INTO public.workers (user_id, profession, main_category, sub_category, experience, cnic, city, service_areas, expertise_tags, latitude, longitude, available, shop_name)
    VALUES (
      NEW.id,
      v_profession,
      v_main_category,
